...
>
    <section class="pb-8 lg:py-12">
        <div class="mx-auto max-w-7xl lg:px-8">
            {{-- Desktop Breadcrumb --}}
            <nav class="hidden items-center gap-2 pb-6 text-sm font-bold text-slate-500 lg:flex">
                <a
                    href="{{ route('home') }}"
                ...
                <span class="max-w-sm truncate text-slate-800">
                    {{ $menuItem->name }}
                </span>
            </nav>
            ...
            <div class="lg:grid lg:grid-cols-[0.92fr_1.08fr] lg:items-start lg:gap-8">
                ...
                <div class="relative lg:sticky lg:top-24">
                    <div class="relative aspect-[5/4] overflow-hidden bg-gradient-to-br from-orange-100 via-amber-50 to-red-100 sm:aspect-[16/10] lg:aspect-[4/5] lg:rounded-[2rem] lg:border lg:border-orange-100 lg:shadow-2xl lg:shadow-orange-900/10">
                        ...
                        @else
                            ...
                            <div class="relative grid h-full place-items-center">
                                <div class="grid h-24 w-24 place-items-center rounded-full bg-white/85 text-5xl font-black text-orange-600 shadow-2xl backdrop-blur sm:h-32 sm:w-32 sm:text-7xl">
                        ...
                        <div class="absolute inset-x-0 bottom-0 h-32 bg-gradient-to-t from-slate-950/65 to-transparent"></div>

                        {{-- Mobile Navigation Over Image --}}
                        <div class="absolute inset-x-0 top-0 z-10 flex items-center justify-between p-4 lg:hidden">
                            <a
                                href="{{ route('menu') }}"
                                class="inline-flex h-11 w-11 items-center justify-center rounded-full border border-white/20 bg-slate-950/35 text-white shadow-lg backdrop-blur-md transition active:scale-95"
                            ...
                            <a
                                ...
                                class="relative inline-flex h-11 w-11 items-center justify-center rounded-full border border-white/20 bg-slate-950/35 text-white shadow-lg backdrop-blur-md transition active:scale-95"
                        ...
                        <div class="absolute bottom-5 left-4 right-4 flex items-end justify-between gap-3 text-white sm:left-6 sm:right-6 lg:bottom-7">
                            <div class="min-w-0">
                                <span class="inline-flex max-w-full truncate rounded-full border border-white/25 bg-slate-950/40 px-3 py-1.5 text-[9px] font-black uppercase tracking-[0.13em] backdrop-blur-md sm:text-xs">
                                    {{ $menuItem->category?->name ?? 'FreshBite Menu' }}
                                </span>
                    ...
                    <div class="mt-4 hidden grid-cols-3 gap-3 lg:grid">
                        @if ($menuItem->preparation_time)
                            <div class="rounded-2xl border border-orange-100 bg-white p-4 text-center shadow-sm">
                        ...
                        @if ($restaurant)
                            <div class="rounded-2xl border border-orange-100 bg-white p-4 text-center shadow-sm">
                                ...
                                <p class="mt-1 text-sm font-black text-slate-950">
                                    Rs. {{ number_format($restaurant->delivery_fee, 0) }}
                                </p>
                            </div>
                        @endif
                    </div>
                ...
                <div class="relative z-20 -mt-7 rounded-t-[2rem] bg-white px-4 pb-7 pt-5 shadow-[0_-10px_30px_rgba(15,23,42,0.08)] sm:-mt-10 sm:px-6 sm:pt-7 lg:mt-0 lg:rounded-[2rem] lg:border lg:border-orange-100 lg:p-9 lg:shadow-sm">
                    {{-- Product Header --}}
                    <header>
                        <div class="flex items-center justify-between gap-3">
                            ...
                            <span
                                @class([
                                    ...
                                    'bg-emerald-50 text-emerald-700' => $canOrder,
                                    'bg-amber-50 text-amber-700' => ! $canOrder,
                                ])
                            >
                                <span
                                    @class([
                                        'h-1.5 w-1.5 rounded-full',
                                        'animate-pulse bg-emerald-500' => $canOrder,
                                        'bg-amber-500' => ! $canOrder,
                                    ])
                                ></span>
                        ...
                        <h1 class="mt-2 break-words text-2xl font-black leading-tight tracking-tight text-slate-950 sm:text-4xl lg:text-5xl">
                            {{ $menuItem->name }}
                        </h1>
                        ...
                        <div class="mt-3 flex flex-wrap items-end gap-x-3 gap-y-2">
                            <p
                                aria-live="polite"
                                class="text-3xl font-black tracking-tight text-orange-600 sm:text-4xl"
                            >
                                Rs.
                            ...
                            @if ($hasDiscount)
                                <p class="pb-1 text-sm font-bold text-slate-400 line-through sm:text-lg">
                                    Rs. {{ number_format($menuItem->compare_at_price, 0) }}
                                </p>

                                <span class="mb-1 rounded-full bg-red-50 px-2.5 py-1 text-[9px] font-black uppercase tracking-[0.1em] text-red-600 sm:text-xs">
                                    Save Rs. {{ number_format($savingAmount, 0) }}
                                </span>
                            @endif
                        </div>
                    ...
                    <div class="mt-5 flex divide-x divide-slate-200 rounded-2xl bg-slate-50 px-2 py-3 lg:hidden">
                        @if ($menuItem->preparation_time)
                            <div class="min-w-0 flex-1 px-2 text-center">
                                <svg
                                    ...
                                    stroke="currentColor"
                                    stroke-width="2"
                                    class="mx-auto h-4 w-4 text-orange-600"
                                >
                                    <circle cx="12" cy="12" r="9" />
                                    <path stroke-linecap="round" d="M12 7v5l3 2" />
                                </svg>

                                <p class="mt-1 text-[8px] font-black uppercase tracking-[0.1em] text-slate-400">
                                    Ready In
                                </p>

                                <p class="mt-0.5 truncate text-xs font-black text-slate-950">
                                    {{ $menuItem->preparation_time }} min
                                </p>
                            </div>
                        @endif
                    ...
                    {{-- Description --}}
                    <p class="mt-5 hidden text-sm font-semibold leading-7 text-slate-600 sm:block">
                        {{ $menuItem->description ?: 'Freshly prepared with carefully selected ingredients for a delicious and satisfying meal.' }}
                    </p>

                    <details class="group mt-4 border-b border-slate-200 pb-4 sm:hidden">
                    ...
                    @unless ($canOrder)
                        <div class="mt-5 rounded-2xl border border-amber-200 bg-amber-50 p-4">
                            <div class="flex items-start gap-3">
                                <span class="grid h-9 w-9 shrink-0 place-items-center rounded-xl bg-white text-amber-600 shadow-sm">
                                    <svg
                                        xmlns="http://www.w3.org/2000/svg"
                                        viewBox="0 0 24 24"
                                        fill="none"
                        ...
                        @if ($sizeOptions->isNotEmpty())
                            <section>
                                <div class="flex items-start justify-between gap-4">
                                    ...
                                    <span class="rounded-full bg-orange-50 px-2.5 py-1 text-[9px] font-black uppercase tracking-[0.1em] text-orange-700">
                                        Required
                                    </span>
                                </div>
                                    ...
                                    @foreach ($sizeOptions as $size)
                                        ...
                                        >
                                            <input
                                                ...
                                                required
                                                class="absolute right-3 top-3 h-5 w-5 border-slate-300 text-orange-600 focus:ring-orange-500"
                                            >

                                            <span class="block pr-7 text-sm font-black text-slate-950">
                                                {{ $size['name'] }}
                                            </span>

                                            <span class="mt-2 block text-base font-black text-orange-600 sm:text-lg">
                                                Rs. {{ number_format($size['price'], 0) }}
                                            </span>
                                        </label>
                                    @endforeach
                                </div>
                            ...
                            <section>
                                <div class="flex items-start justify-between gap-4">
                                    ...
                                    <span
                                        ...
                                        x-cloak
                                        class="rounded-full bg-orange-50 px-2.5 py-1 text-[9px] font-black text-orange-700"
                                    >
                                        ...
                                        >
                                            <span class="flex min-w-0 items-center gap-3">
                                                <input
                                                    ...
                                                    class="h-5 w-5 shrink-0 rounded-md border-slate-300 text-orange-600 focus:ring-orange-500"
                                                >
                                                ...
                                                <span class="min-w-0">
                                                    ...
                                                    <span class="mt-0.5 block text-[9px] font-bold uppercase tracking-[0.1em] text-slate-400">
                                                        {{ ucfirst($addon['type']) }}
                                                    </span>
                                                </span>
                                            </span>
                        ...
                        @if ($sizeOptions->isNotEmpty() || $addonOptions->isNotEmpty())
                            <div class="rounded-2xl border border-orange-100 bg-orange-50 p-4">
                                <div class="flex items-start justify-between gap-4">
                                    ...
                                    <p class="shrink-0 text-lg font-black text-orange-600">
                                        Rs. <span x-text="formatPrice(unitPrice)"></span>
                                    </p>
                                </div>
                            </div>
                        @endif
                        ...
                        <div class="hidden gap-3 lg:grid lg:grid-cols-[1fr_auto]">
                            @if ($canOrder)
                                <button
                                    ...
                                    class="inline-flex min-h-14 w-full items-center justify-center gap-2 rounded-2xl bg-orange-600 px-6 py-4 text-sm font-black text-white shadow-lg shadow-orange-600/20 transition hover:bg-orange-700 disabled:cursor-not-allowed disabled:opacity-70"
                                >
                                    <svg
                                        x-show="submitting"
                                        x-cloak
                            ...
                            @else
                                <button
                                    type="button"
                                    disabled
                                    class="inline-flex min-h-14 w-full cursor-not-allowed items-center justify-center rounded-2xl bg-slate-200 px-6 py-4 text-sm font-black text-slate-500"
                                >
                            ...
                            <a
                                href="{{ route('menu') }}"
                                class="inline-flex min-h-14 items-center justify-center rounded-2xl border border-orange-200 bg-white px-6 py-4 text-sm font-black text-slate-800 shadow-sm transition hover:bg-orange-50 hover:text-orange-700"
                            >
                                Back to Menu
                            </a>
                        </div>
    ...
    @if (($suggestions ?? collect())->isNotEmpty())
        <section class="border-t border-orange-100 bg-white py-8 sm:py-12">
            <div class="mx-auto max-w-7xl px-4 sm:px-6 lg:px-8">
                <div class="flex items-end justify-between gap-4">
                    <div>
                        ...
                        <h2 class="mt-1 text-xl font-black tracking-tight text-slate-950 sm:text-3xl">
                            Popular pairings
                        </h2>
                    </div>
                ...
                <div class="mt-5 grid gap-3 lg:grid-cols-4">
                    @foreach ($suggestions as $suggestion)
                        ...
                        <article class="flex items-center gap-3 rounded-2xl border border-orange-100 bg-white p-3 shadow-sm lg:block lg:overflow-hidden lg:p-0">
                            <a
                                href="{{ route('menu.show', $suggestion) }}"
                                class="h-20 w-20 shrink-0 overflow-hidden rounded-xl bg-orange-50 lg:block lg:h-auto lg:w-auto lg:rounded-none"
                            >
                            ...
                            <div class="min-w-0 flex-1 lg:p-4">
                                <a
                                    href="{{ route('menu.show', $suggestion) }}"
                                    class="line-clamp-1 text-sm font-black text-slate-950 hover:text-orange-700 lg:text-base"
                                >
                                    {{ $suggestion->name }}
                                </a>
                                ...
                                <div class="mt-2">
                                    @if ($suggestionCustomizable)
                                        <a
                                            href="{{ route('menu.show', $suggestion) }}"
                                            class="inline-flex min-h-9 items-center justify-center rounded-lg bg-orange-50 px-3 py-2 text-[10px] font-black text-orange-700 transition active:scale-[0.97]"
                                        >
                                            Customize
                                        </a>
                                    @else
                                        ...
                                        >
                                            ...
                                            <button
                                                type="submit"
                                                class="inline-flex min-h-9 items-center justify-center gap-1 rounded-lg bg-orange-600 px-3 py-2 text-[10px] font-black text-white transition active:scale-[0.97]"
                                            >
                                                <svg
                                                    xmlns="http://www.w3.org/2000/svg"
                                                    viewBox="0 0 24 24"
    ...
    @if ($relatedItems->isNotEmpty())
        <section class="border-t border-orange-100 py-8 sm:py-12">
            <div class="mx-auto max-w-7xl px-4 sm:px-6 lg:px-8">
                <div class="flex items-end justify-between gap-4">
                    <div>
                        <p class="text-[10px] font-black uppercase tracking-[0.18em] text-orange-600 sm:text-xs">
                ...
                <div class="mt-5 grid gap-3 lg:grid-cols-3">
                    @foreach ($relatedItems as $item)
                        <a
                            ...
                            class="group flex items-center gap-3 rounded-2xl border border-orange-100 bg-white p-3 shadow-sm transition active:scale-[0.99] lg:block lg:overflow-hidden lg:p-0"
                        >
                            <div class="h-20 w-20 shrink-0 overflow-hidden rounded-xl bg-orange-50 lg:h-auto lg:w-auto lg:rounded-none">
                            ...
                            <div class="min-w-0 flex-1 lg:p-5">
                                <h3 class="line-clamp-1 text-sm font-black text-slate-950 lg:text-lg">
                                    {{ $item->name }}
                                </h3>

                                <p class="mt-1 line-clamp-1 text-xs font-semibold text-slate-500 lg:line-clamp-2 lg:text-sm">
                                    {{ $item->description ?: 'Freshly prepared with quality ingredients.' }}
                                </p>

                                <div class="mt-2 flex items-center justify-between gap-3">
                                    <p class="text-sm font-black text-orange-600 lg:text-lg">
                                        Rs. {{ number_format($item->price, 0) }}
                                    </p>

                                    <span class="grid h-8 w-8 shrink-0 place-items-center rounded-lg bg-orange-50 text-orange-600">
                                        <svg
                                            xmlns="http://www.w3.org/2000/svg"
                                            viewBox="0 0 24 24"
                                            fill="none"
    ...
    <div class="fixed inset-x-0 bottom-0 z-50 border-t border-orange-100 bg-white/95 px-4 pt-3 shadow-[0_-12px_30px_rgba(15,23,42,0.14)] backdrop-blur lg:hidden">
        <div class="mx-auto flex items-center gap-3 pb-[calc(0.75rem+env(safe-area-inset-bottom))]">
            <div class="min-w-0 shrink-0">
                ...
                <p
                    aria-live="polite"
                    class="mt-0.5 whitespace-nowrap text-lg font-black text-slate-950"
                >
                    Rs. <span x-text="formatPrice(unitPrice)"></span>
                </p>
            </div>
