...
<title>{{ isset($title) ? $title . ' | FreshBite Rider' : 'FreshBite Rider' }}</title>
...
<body class="min-h-screen bg-[var(--color-surface-app)] font-sans text-slate-900 antialiased">
    ...
    >
        ...
        <header class="sticky top-0 z-50 border-b border-orange-100 bg-white/95 shadow-sm backdrop-blur-xl lg:hidden">
            <div class="flex items-center justify-between px-4 py-3">
                <a href="{{ route('rider.dashboard') }}" class="flex min-w-0 items-center gap-3">
                    <span class="grid h-11 w-11 shrink-0 place-items-center rounded-2xl bg-gradient-to-br from-orange-500 to-red-600 text-sm font-black text-white shadow-lg shadow-orange-600/20">
                        FB
                    </span>

                <span class="min-w-0">
                    <span class="block truncate text-base font-black tracking-tight text-slate-950">
                        FreshBite
                    </span>
            ...
            <button
                ...
                class="grid h-11 w-11 shrink-0 place-items-center rounded-2xl border border-orange-200 bg-white text-slate-700 shadow-sm transition hover:bg-orange-50"
        ...
        <div
            ...
            x-transition
            x-cloak
            class="border-t border-orange-100 bg-white px-4 py-4 shadow-xl"
        >
            <div class="mb-4 flex items-center gap-3 rounded-2xl border border-orange-100 bg-[var(--color-surface-warm)] p-4">
                <div class="grid h-12 w-12 shrink-0 place-items-center rounded-2xl bg-orange-600 text-sm font-black text-white">
            ...
            <nav class="grid gap-2 text-sm font-bold">
                <a
                    ...
                    class="flex items-center gap-3 rounded-2xl px-4 py-3 transition {{ request()->routeIs('rider.dashboard') ? 'bg-orange-600 text-white shadow-lg shadow-orange-600/20' : 'text-slate-700 hover:bg-orange-50 hover:text-orange-700' }}"
                >
                ...
                <a
                    ...
                    class="flex items-center gap-3 rounded-2xl px-4 py-3 transition {{ request()->routeIs('rider.orders*') ? 'bg-orange-600 text-white shadow-lg shadow-orange-600/20' : 'text-slate-700 hover:bg-orange-50 hover:text-orange-700' }}"
                >
                    <svg xmlns="http://www.w3.org/2000/svg" class="h-5 w-5 shrink-0" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2">
    ...
    <div class="min-h-screen lg:pl-[290px]">
        {{-- Desktop Sidebar --}}
        <aside class="fixed bottom-0 left-0 top-0 z-40 hidden w-[290px] overflow-hidden border-r border-orange-100 bg-white p-5 shadow-sm shadow-orange-900/5 lg:block">
            <div class="flex h-full flex-col overflow-y-auto pr-1">
                ...
                <div class="mt-auto">
                    <div class="rounded-[1.5rem] border border-orange-100 bg-gradient-to-br from-orange-50 to-red-50 p-4">
                        <div class="grid h-11 w-11 place-items-center rounded-2xl bg-white text-orange-600 shadow-sm">
        ...
        <main class="min-w-0 pb-24 lg:pb-0">
            ...
            <header class="sticky top-0 z-40 hidden border-b border-orange-100 bg-white/90 px-6 py-4 shadow-sm shadow-orange-900/5 backdrop-blur-xl lg:block">
                <div class="flex items-center justify-between gap-6">
                    ...
                    <div class="flex items-center gap-3">
                        <div class="flex items-center gap-2 rounded-2xl border border-emerald-100 bg-emerald-50 px-4 py-2.5">
                            <span class="h-2 w-2 rounded-full bg-emerald-500"></span>
    ...
    <nav class="fixed inset-x-0 bottom-0 z-50 border-t border-orange-100 bg-white/95 px-3 py-2 shadow-[var(--shadow-bottom-nav)] backdrop-blur-xl lg:hidden">
        <div class="mx-auto grid max-w-md grid-cols-3 gap-2">
            <a
                href="{{ route('rider.dashboard') }}"
                class="flex flex-col items-center justify-center gap-1 rounded-2xl px-3 py-2 text-xs font-black transition {{ request()->routeIs('rider.dashboard') ? 'bg-orange-50 text-orange-700' : 'text-slate-500' }}"
            >
            ...
            <a
                href="{{ route('rider.orders') }}"
                class="flex flex-col items-center justify-center gap-1 rounded-2xl px-3 py-2 text-xs font-black transition {{ request()->routeIs('rider.orders*') ? 'bg-orange-50 text-orange-700' : 'text-slate-500' }}"
            >
                <svg xmlns="http://www.w3.org/2000/svg" class="h-5 w-5" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2">
                    <path d="M3 7h11v10H3z" />
                    <path d="M14 10h4l3 3v4h-7z" />
            ...
            <a
                href="{{ route('account.security') }}"
                class="flex flex-col items-center justify-center gap-1 rounded-2xl px-3 py-2 text-xs font-black transition {{ request()->routeIs('account.security') ? 'bg-orange-50 text-orange-700' : 'text-slate-500' }}"
            >
                <svg xmlns="http://www.w3.org/2000/svg" class="h-5 w-5" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2">
                    <path d="M12 3 5 6v5c0 4.5 3 8.5 7 10 4-1.5 7-5.5 7-10V6l-7-3z" />
                    <path d="M9.5 12.5 11 14l3.5-4" />
